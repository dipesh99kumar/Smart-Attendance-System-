...
def detect(gray, frame):
   ...
   for (x,y,w,h) in faces:
        ...
        cv2.putText(frame,"ROLL : "+str(ID)+"  ",(25,470), cv2.FONT_HERSHEY_COMPLEX, 1.0, (255, 255, 255), 3)
        roi_gray=gray[y:y+h,x:x+w]
        roi_color=frame[y:y+h,x:x+w]
        id1=int(ID)
        if (id1<35):
            sql3="UPDATE ATTENDANCE SET TOTAL=TOTAL+1 WHERE PRESENT_ABSENT='A' AND SLNO=%d"%(id1)
            cursor.execute(sql3)
            sql2="UPDATE ATTENDANCE SET PRESENT_ABSENT='P' WHERE SLNO=%d"%id1
            cursor.execute(sql2)
...
#sql1="SELECT NAME FROM ATTENDANCE WHERE SLNO= %d"%(ID)
#cursor.execute(sql1)
